...
export const completeOnboarding = async (req, res) => {
    try {
        const { Fullname, password, required } = req.body
        const user = await User.findById(req.user._id);
        const profilePic = req.body.profilePic|| null;

        if (!user) return res.status(404).json({ message: "User not found" });
        if (user.isOnboarded) return res.status(400).json({ message: "Already onboarded" });
        ...
        if (profilePic && profilePic.startsWith("data:image")) {
            ...
            user.profilePic = profilePic; // ✅ Google photo URL — save directly
        }
        else {
        ...
        console.log("Error in completeOnboarding:", error.message);
