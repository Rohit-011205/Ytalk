...
const ChatHeader = () => {
  ...
  return (
    <div className="p-4 border-b border-white/5 bg-[#050208]/80 backdrop-blur-md">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          
          {/* BACK BUTTON: Only visible on mobile (lg:hidden) */}
          <button 
            onClick={() => setSelectedUser(null)} 
          ...
          <div className="avatar">
            <div className="size-10 rounded-full relative border border-white/10">
              <img src={selectedUser.profilePic || "/avatar.png"} alt={selectedUser.Fullname} />
            </div>
          </div>

          <div>
            <h3 className="font-medium text-purple-50">{selectedUser.Fullname}</h3>
            <p className="text-xs text-purple-500/60">
              {/* Add online status logic here if you have it */}
              Click for info
            </p>
          </div>
        </div>
        
        {/* Rest of your header icons (Phone, Video, etc) */}
      </div>
    </div>
  );
};
