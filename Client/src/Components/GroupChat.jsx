...
import { useVideoCallStore } from "../Store/useVideoCall.js";
import reactLogo from "../assets/react.svg";
import { Video, Info, ArrowLeft } from "lucide-react"; // Added ArrowLeft
import GroupSettingModal from "./GroupSettingModal";

const GroupChat = () => {
    const {
        activeGroup,
        setActiveGroup, // Destructured this to use for the back button
        groupMessages,
        fetchGroupMessages,
        sendGroupMessage,
        subscribeToGroupMessages,
    ...
    return (
        <div className="flex flex-1 flex-col h-full overflow-hidden bg-[#050208]">
            {/* GROUP HEADER */}
            <div className="border-b border-white/5 px-4 lg:px-6 py-4 bg-[#050208]/80 backdrop-blur-md">
                <div className="flex items-center gap-3">
                    
                    {/* BACK BUTTON: Only shows on mobile */}
                    <button 
                        onClick={() => setActiveGroup(null)}
                        className="lg:hidden p-2 -ml-2 text-purple-400 hover:text-purple-300 transition-colors"
                    >
                        <ArrowLeft className="size-6" />
                    </button>

                    <div className="size-10 rounded-full bg-purple-900/30 flex items-center justify-center border border-purple-500/20 text-purple-400 font-bold flex-shrink-0">
                        {activeGroup.name.charAt(0).toUpperCase()}
                    </div>
                    
                    <div className="min-w-0">
                        <h2 className="text-white font-semibold tracking-tight truncate">
                            {activeGroup.name}
                        </h2>
                        <p className="text-[11px] text-zinc-500 uppercase tracking-wider">
                            {activeGroup.members?.length || 0} Members
